...
class DatasetGeneratorNode(Node):
    def __init__(self):
        ...
        #self.ee_frame_id = self.model.getFrameId("gripper") # Le bout du bras
        ...
        self.video_writer_base = None
        self.video_writer_pince = None
        # Format mp4 standard (vous pouvez aussi utiliser 'XVID' pour du .avi)
        


        # --- PUBLISHER (Téléportation Gazebo Ignition) ---
        # Remarque : Adaptez ce topic si votre pont ROS/Gazebo pour la pose de la boîte a un nom différent
        #self.box_pose_pub = self.create_publisher(Pose, '/model/red_box/pose', 10)

# ==========================================
    # CALLBACKS DES CAMÉRAS (ENREGISTREMENT CONTINU)
    # ==========================================
    def cam_base_cb(self, msg):
        self.latest_img_base = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        # Si l'enregistrement est activé, on ajoute la frame à la vidéo
        if self.is_recording and self.video_writer_base is not None:
            self.video_writer_base.write(self.latest_img_base)

    def cam_pince_cb(self, msg):
        self.latest_img_pince = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        if self.is_recording and self.video_writer_pince is not None:
            self.video_writer_pince.write(self.latest_img_pince)
    ...
    # ==========================================
    # CONTRÔLE DES MOTEURS (Votre code)
    # ==========================================
    ...
    def run_dataset_loop(self, num_episodes=10):
        # 0. On force la réception des toutes premières images pour connaître leur taille exacte
        while self.latest_img_base is None or self.latest_img_pince is None:
            rclpy.spin_once(self, timeout_sec=0.1)
            
        # On extrait la VRAIE résolution (Largeur, Hauteur)
        h_b, w_b, _ = self.latest_img_base.shape
        h_p, w_p, _ = self.latest_img_pince.shape
        
        # Codec pare-balle pour Linux
        ...
        for ep in range(num_episodes):
            ...
            # On passe en .avi !
            base_vid_path = f"{self.dataset_dir}/ep{ep}_base.avi"
            pince_vid_path = f"{self.dataset_dir}/ep{ep}_pince.avi"
            
            # On utilise la vraie résolution mesurée (w_b, h_b)
            self.video_writer_base = cv2.VideoWriter(base_vid_path, fourcc, 30.0, (w_b, h_b))
            self.video_writer_pince = cv2.VideoWriter(pince_vid_path, fourcc, 30.0, (w_p, h_p))
            ...
            #place_pos = self.generate_random_target()
            ...
            """#=================AJOUT CIBLE VERTE POUR DEBUGG====================================
            # 1. Tentative de création (pour le premier épisode)
            sdf_target = "/home/polytech/Documents/Stage_ISIR/ROS/lerobot_ws/src/lerobot_description/urdf/target.sdf"
            cmd_target = [
                "ros2", "run", "ros_gz_sim", "create",
                "-file", sdf_target,
                "-name", "cible_verte",  
                "-x", str(pick_pos[0]),
                "-y", str(pick_pos[1]),
                "-z", str(pick_pos[2] + 0.15)
            ]
            subprocess.run(cmd_target, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

            # 2. Téléportation (pour tous les épisodes suivants)
            self.teleport_entity_ign("cible_verte", pick_pos[0], pick_pos[1], pick_pos[2] + 0.15)"""
            ...
            # --- 3. ARRÊTER L'ENREGISTREMENT VIDÉO ---
            self.is_recording = False
            self.video_writer_base.release()
            self.video_writer_pince.release()
            
            print(f"===== FIN DE L'ÉPISODE {ep} (Vidéos sauvegardées) =====")
